...
- (void)setupViewsStyle {
    ...
    [self.passwordTextField setLayerBorderWidth:.5 color:[UIColor borderColor]];
    [self.passwordTextField setLayerCornerRadius:4];
    [self.passwordTextField setTextFieldLeftPaddingWidth:5];
...
- (IBAction)submitBtnAction:(id)sender {
    if (kIsObjectEmpty(self.mobileTxtF.text)) {
        [NSString toast:@"请输入手机号码"];
        return;
    }
    if (kIsObjectEmpty(self.passwordTextField.text)) {
        [NSString toast:@"请输入要重置的新密码"];
        return;
    }
    if (kIsObjectEmpty(self.codeTxtF.text)) {
...
#pragma mark --用于修改密码
- (void)sendSmsdoUpdatePwd {
    NSMutableDictionary *params = [NSMutableDictionary dictionary];
    [params addUnEmptyString:self.mobileTxtF.text forKey:@"mobile"];
    [params addUnEmptyString:self.passwordTextField.text forKey:@"password"];
WEAKSELF
    [UserInfoObj sendSmsdoUpdateWithParameters:params successBlock:^(HttpRequest *request, HttpResponse *response) {
